    ...
    if file_option == "CSV Upload":
        ...
        if uploaded_file:
            ...
            if st.button("Preprocess and Save", key="preprocess"):
                with st.spinner("Preprocessing tickets..."):
                    processor = TicketProcessor(uploaded_file, "data/processed/preprocessed_tickets6.csv")
                    processor.process_and_save()
                    st.success("Preprocessing complete! Saved to data/processed/preprocessed_tickets6.csv")
        ...
        if st.button("Preprocess and Save Default File"):
            with st.spinner("Preprocessing default tickets..."):
                processor.process_and_save()
                st.success("Preprocessing complete! Processed file saved.")
...
if page == "🎫 Ticket Classification":
    ...
    if classify_option == "Single Ticket":
        text_input = st.text_area("Enter a support ticket:")
        if st.button("Classify Ticket", key="single_ticket"):
            with st.spinner("Classifying..."):
                st.session_state.ticket_classify_result = classifier.classify_ticket(text_input)
            ...
            if "clean_text" not in df.columns:
                ...
                if st.button("Classify All Tickets", key="batch_ticket"):
                    with st.spinner("Classifying all tickets..."):
                        ...
                        tickets_file_path = os.path.join(classified_data_dir, "classified_tickets6.csv")
...
if page == "📄 Ticket Recommendations":
    st.header("📄 Generate Recommendation for Single Ticket")
    ...
    if submitted:
        try:
            ...
            st.code("uvicorn src.recommend_api2:app --reload")
    ...
    if st.session_state.recommendation_result:
        ...
        if os.path.exists(LOG_PATH):
            recs.to_csv(LOG_PATH, index=False)  # Log recommendations to CSV

    st.markdown("---")

    st.header("📄 Generate Recommendation for Multiple Tickets")
    ...
    if uploaded_file:
        ...
        if st.button("Get Recommendations ⚡", key="recommend_button"):
            with st.spinner("Generating Recommendations..."):
                ...
                results_df.to_csv("logs/recommendation_results5.csv", index=False)
                st.success("✅ Recommendations generated and saved to logs/recommendation_results5.csv")
